...
public class PlayListController extends PanelController implements
	...
	protected void addBindings() {
		final PlayListTable table = ((PlayListPanel) panelControlled)
				.getPlayListTable();
		...
		// Keys
		table.getDeleteItem().setAccelerator(
				KeyStroke.getKeyStroke(KeyEvent.VK_DELETE, 0));
		table.getPlayItem().setAccelerator(
				KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0));
		table.getInfoItem().setAccelerator(
				KeyStroke.getKeyStroke(KeyEvent.VK_F1, 0));
		table.getSaveItem().setAccelerator(
				KeyStroke.getKeyStroke(KeyEvent.VK_S, ActionEvent.CTRL_MASK));
		table.getLoadItem().setAccelerator(
				KeyStroke.getKeyStroke(KeyEvent.VK_L, ActionEvent.CTRL_MASK));
		table.getClearItem().setAccelerator(
				KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0));
		table.getTopItem().setAccelerator(
				KeyStroke.getKeyStroke(KeyEvent.VK_T, ActionEvent.CTRL_MASK));
		table.getDownItem()
				.setAccelerator(
						KeyStroke.getKeyStroke(KeyEvent.VK_DOWN,
								ActionEvent.CTRL_MASK));
		table.getUpItem().setAccelerator(
				KeyStroke.getKeyStroke(KeyEvent.VK_UP, ActionEvent.CTRL_MASK));
		table.getBottomItem().setAccelerator(
				KeyStroke.getKeyStroke(KeyEvent.VK_B, ActionEvent.CTRL_MASK));
		table.getFavoriteSong().setAccelerator(
				KeyStroke.getKeyStroke(KeyEvent.VK_F2, 0));
		table.getFavoriteAlbum().setAccelerator(
				KeyStroke.getKeyStroke(KeyEvent.VK_F3, 0));
		table.getFavoriteArtist().setAccelerator(
	...
	public void updatePositionInTable(int pos) {
		((PlayListTableModel) ((PlayListPanel) panelControlled)
				.getPlayListTable().getModel()).refresh(pos);
	}

	public void addSongsToPlayList(ArrayList songs, int selectedSong) {
		if (selectedSong == -1)
			selectedSong = ((PlayListPanel) panelControlled).getPlayListTable()
					.getPlayingSong();

		for (int i = 0; i < songs.size(); i++)
			((PlayListTableModel) ((PlayListPanel) panelControlled)
					.getPlayListTable().getModel()).addSong((AudioFile) songs
					.get(i));

		setSelectedSongOnTable(selectedSong);

		HandlerProxy.getControllerHandler().getPlayListControlsController()
				.enableSaveButton(true);
		HandlerProxy.getControllerHandler().getMenuController()
				.enableSavePlaylist(true);
	}
	...
	private void setSelectedSongOnTable(int song) {
		PlayListTable table = ((PlayListPanel) panelControlled)
				.getPlayListTable();
	...
	public void adjustColumnsWidth() {
		...
		boolean showTrack = ((PlayListTableModel) table.getModel())
				.isTrackVisible();
		boolean showGenre = ((PlayListTableModel) table.getModel())
				.isGenreVisible();
		...
		int nonDurationWidth = tableWidth - favoriteColumnWidth
				- trackColumnWidth - durationColumnWidth;
		int nonDurationColumns = table.getColumnCount() - 3
				- (showGenre ? 1 : 0);
		...
		if (showGenre) {
			table.getColumnModel()
					...
					.setMinWidth(genreColumnWidth);
			table.getColumnModel()
					.getColumn(table.getColumnModel().getColumnCount() - 2)
					.setMaxWidth(genreColumnWidth);
		}

		table.getColumnModel()
				.getColumn(table.getColumnModel().getColumnCount() - 1)
				.setMinWidth(durationColumnWidth);
		table.getColumnModel()
				.getColumn(table.getColumnModel().getColumnCount() - 1)
				.setMaxWidth(durationColumnWidth);

		for (int i = (showGenre ? 2 : 1); i < nonDurationColumns; i++) {
			table.getColumnModel().getColumn(i)
					.setPreferredWidth(nonDurationWidth / nonDurationColumns);
		}
		table.invalidate();
	...
	public void playSelectedSong() {
		int song = ((PlayListPanel) panelControlled).getPlayListTable()
				.getSelectedRow();
	...
	public void setTrackNumber() {
		PlayListTable table = HandlerProxy.getVisualHandler()
				.getPlayListTable();
		int[] rows = table.getSelectedRows();
		ArrayList<AudioFile> files = new ArrayList<AudioFile>();
		for (int i = 0; i < rows.length; i++) {
			files.add(((PlayListTableModel) table.getModel())
					.getFileAt(rows[i]));
		}
		HashMap<AudioFile, Integer> filesAndTracks = RegexpUtils
				.getFilesAndTrackNumbers(files);
	...
	public void setGenre() {
		...
		HashMap<AudioFile, String> filesAndGenres = HandlerProxy
				.getAudioScrobblerServiceHandler().getGenresForFiles(files);
	...
	public void moveToTop() {
		...
		if (rows.length > 0 && rows[0] > 0) {
			...
			((PlayListPanel) panelControlled).getPlayListTable()
					.getSelectionModel()
					.setSelectionInterval(0, rows.length - 1);
		}
		// Solves bug 1663521
		adjustColumnsWidth();
	}
	...
	public void moveUp() {
		...
		if (rows.length > 0 && rows[0] > 0) {
			...
			((PlayListPanel) panelControlled)
					.getPlayListTable()
					.getSelectionModel()
					.setSelectionInterval(rows[0] - 1,
		...
		if (rows.length > 0 && rows[rows.length - 1] < table.getRowCount() - 1) {
			...
			((PlayListPanel) panelControlled)
					...
					.setSelectionInterval(
							((PlayListPanel) panelControlled)
									.getPlayListTable().getRowCount()
									- rows.length,
							((PlayListPanel) panelControlled)
									.getPlayListTable().getRowCount() - 1);
		}
		// Solves bug 1663521
		adjustColumnsWidth();
	}
	...
	private ArrayList<AudioFile> getSelectedSongs(JTable table) {
		...
		for (int i = 0; i < selectedRows.length; i++) {
			AudioFile file = ((PlayListTableModel) table.getModel())
					.getFileAt(selectedRows[i]);
			songs.add(file);
		}
		return songs;
	}
	...
	public ArrayList<AudioFile> getSelectedSongs() {
		return getSelectedSongs(HandlerProxy.getVisualHandler()
				.getPlayListTable());
	}
	...
	public void setArtistAsPlaylist() {
		String artist = HandlerProxy.getPlayerHandler().getCurrentPlayList()
				.getCurrentFile().getArtist();
		HashMap<String, Artist> structure = HandlerProxy.getRepositoryHandler()
	...
	public void setAlbumAsPlaylist() {
		String artist = HandlerProxy.getPlayerHandler().getCurrentPlayList()
				.getCurrentFile().getArtist();
		String album = HandlerProxy.getPlayerHandler().getCurrentPlayList()
				.getCurrentFile().getAlbum();
		HashMap<String, Artist> structure = HandlerProxy.getRepositoryHandler()
				.getArtistAndAlbumStructure();
		if (structure.containsKey(artist)) {
			Artist a = structure.get(artist);
	...
	public void setDurationAsPlaylist() {
		String artist = HandlerProxy.getPlayerHandler().getCurrentPlayList()
				.getCurrentFile().getArtist();
		String album = HandlerProxy.getPlayerHandler().getCurrentPlayList()
				.getCurrentFile().getAlbum();
	...
	public void showPlaylistControls(boolean show) {
		...
		HandlerProxy.getControllerHandler().getMenuController()
				.setShowPlaylistControls(show);
		((PlayListPanel) panelControlled).getPlayListControls()
				.setVisible(show);
	}
}
